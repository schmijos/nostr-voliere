Feature: NIP-01 - Client to relay communication - Filtering events
  ...
  Scenario: Filtering on event id
    ...
    """json
    ["REQ", "A", { "ids": ["93421380eaff3494b81293eafb0f3bd36c1d54770469a6626085d6597624372a"] }]
    """
  ...
  Scenario: Filtering on multiple event ids
    When the client sends
    """json
    ["REQ", "A", { "ids": ["93421380eaff3494b81293eafb0f3bd36c1d54770469a6626085d6597624372a", "191795c5ff686a83b86358ab7a8f52651b8375007c2379f6b21d30617868b6c3"] }]
    """
    Then the server responds with event A
    And the server responds with EOSE
  ...
  Scenario: Filtering on author public key
    When the client sends
    """json
    ["REQ", "A", { "authors": ["3f770d65d3a764a9c5cb503ae123e62ec7598ad035d836e2a810f3877a745b24"] } ]
    """
  ...
  Scenario: Filtering on referenced event id (tag)
    When the client sends
    """json
    ["REQ", "A", { "#e": ["5f764d5fc90b41615242099ffa5285be213a226ac360ab8f332cb9399060a00b"] } ]
    """
    Then the server responds with event A
    And the server responds with EOSE
  ...
  Scenario: Filtering on referenced public key (tag)
    When the client sends
    """json
    ["REQ", "A", { "#p": ["958b754a1d3de5b5eca0fe31d2d555f451325f8498a83da1997b7fcd5c39e88c"] } ]
    """
    Then the server responds with event A
    And the server responds with EOSE
  ...
  Scenario: Using multiple conditions in a filter
    When the client sends
    """json
    ["REQ", "A", { "until": 1704400000, "kind": [1], "#p": ["7e2ffb634a5dbc41d36504b0ae78708f137b8c07272d9c91f8a7c900582a4736"] } ]
    """
    Then the server responds with event A
    And the server responds with EOSE
